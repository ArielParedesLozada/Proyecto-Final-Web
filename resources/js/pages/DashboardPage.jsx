...
export default function DashboardPage() {
  ...
  return (
    <AppLayout>
      {/* Stats: filas de altura uniforme */}
      <section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 auto-rows-fr">
        <div className="h-full"><Stat title="Total Ahorrado" value={`$${totalAhorrado.toLocaleString()}`} helper="+12% desde el mes pasado" icon={<span className="text-sm opacity-70">$</span>} /></div>
        <div className="h-full"><Stat title="Meta Mensual" value={`$${metaMensual.toLocaleString()}`} helper="$1250 este mes" icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M3 12h18M3 6h18M3 18h18" className="stroke-current/70" strokeWidth="1.6" strokeLinecap="round"/></svg>} /></div>
        <div className="h-full"><Stat title="Metas Activas" value={metasActivas} helper="2 completadas" icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M3 12l6 6L21 6" className="stroke-current/70" strokeWidth="2" strokeLinecap="round"/></svg>} /></div>
